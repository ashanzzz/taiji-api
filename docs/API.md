...
The service never runs tools. A client such as Codex must execute them and post a later `role: tool` message with the matching `tool_call_id`. The adapter limits each tool result to 24,000 characters. `tool_choice` supports `none`, `auto`, `required`, and forced function selection. Model outputs are wrapped in explicit `<TOOL_CALL>` markers and strictly validated against function parameter JSON Schemas. Streamed tool calls emit incremental `index` markers. Illegal tool intent does not degrade into ordinary text and triggers up to one automatic repair before failing with HTTP 502 `tool_parse_error`. Multi-turn histories preserve `tool_call_id` mappings to prevent parallel tool ambiguities. Bridge streaming is completion-buffered, not token-by-token. Do not use it for public or unattended execution.
