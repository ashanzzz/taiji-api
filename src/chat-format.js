...
export function buildTaijiInput(messages, bridge = null) {
  ...
  const labels = { system: '系统', developer: '开发者', user: '用户', assistant: '助手', tool: '工具' };
...
function normalizeMessage(message, index, calls) {
  if (!message || !['user', 'assistant', 'system', 'developer', 'tool'].includes(message.role)) throw new HttpError(400, `Invalid message role at ${index}`);
  if (message.role === 'tool') {
    const name = calls.get(message.tool_call_id);
    if (!name) throw new HttpError(400, 'Tool result has no matching tool_call_id');
    return { role: 'tool', text: `【${name} 工具结果】\n${textContent(message.content)}`, files: [] };
  }
  if (message.tool_calls) return { role: 'assistant', text: `【工具调用请求】\n${JSON.stringify({ tool_calls: message.tool_calls })}`, files: [] };
